...
int main(void)
{
    ...
    for (i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of 1 %d\n", i);
    ...
    for (i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of 2 %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
            exit(0);
        }
    ...
    for (i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of 3 %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
            exit(0);
        }
    ...
    printf("fjehfjkfdlk.jklsjalshfdlsanahudsals");
    fclose(output);
    fclose(fp);
    return 0;
}
